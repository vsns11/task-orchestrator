...
#   ./inject-flow.sh reset             # truncate orchestrator tables (DB only, safe while app runs)
...
Actions:
  ...
  reset                     truncate orchestrator tables (DB only — safe while app runs)
...
To wipe Kafka topics (run BEFORE the app starts):
  ./scripts/pre-clear.sh
